<template>
  <div class="relative">
    ...
    <button @click="toggleDropdown" :disabled="loading"
    ...
    <transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 scale-95"
      ...
      <div v-if="isOpen"
        ...
        <div v-else-if="error" class="p-3 text-center">
          ...
          <button @click="fetchWorkspaces" class="text-xs text-purple-600 hover:text-purple-700 mt-1">
            Retry
          </button>
        </div>
        ...
        <div v-else-if="workspaces.length > 0" class="py-1">
          <button v-for="workspace in workspaces" :key="workspace.id" @click="selectWorkspace(workspace)"
            :disabled="switching || workspace.id === currentWorkspace?.id"
            class="w-full px-3 py-2 text-left hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed" :class="{
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center space-x-2 min-w-0">
                <div class="w-2 h-2 rounded-full flex-shrink-0"
                  :class="workspace.id === currentWorkspace?.id ? 'bg-purple-500' : 'bg-gray-300'"></div>
        ...
        <div class="border-t border-gray-100 p-2">
          <button @click="fetchWorkspaces" :disabled="loadingWorkspaces"
